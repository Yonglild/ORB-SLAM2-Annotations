...
public:
    ...
    long unsigned int mnBALocalForKF;  //该关键帧参与局部BA时的当前关键帧id
    long unsigned int mnBAFixedForKF;

    // Variables used by the keyframe database
    long unsigned int mnLoopQuery;  //可能闭环关联的帧id
    int mnLoopWords;    //记录pKFi与pKF具有相同word的个数
    float mLoopScore;   //pKFi回环分数（分数越高，月有可能与pKF帧发生回环）
    ...
    cv::Mat mTcwGBA;
    cv::Mat mTcwBefGBA;
    long unsigned int mnBAGlobalForKF;   //BA时，全局的关键帧数

    // Calibration parameters
    const float fx, fy, cx, cy, invfx, invfy, mbf, mb, mThDepth;//mThDepth该帧地图点的深度阈值？？深度大于该值的点，不合格
...
protected:
    ...
    std::vector<MapPoint*> mvpMapPoints;   //该关键帧上的地图点
    ...
    std::vector< std::vector <std::vector<size_t> > > mGrid;   //size_t 每个格子分配的特征点数
    ...
    std::set<KeyFrame*> mspChildrens;   //子关键帧集合
    std::set<KeyFrame*> mspLoopEdges;   //可能与该帧构成回环的关键帧？？
