...
public:
    ...
    ORBextractor* mpORBextractorLeft, *mpORBextractorRight;   //特征点提取器
    ...
    static float invfx;
    static float invfy;
    cv::Mat mDistCoef;  //畸变系数（k1,k2,p1,p2等）

    // Stereo baseline multiplied by fx.
    float mbf;     //z=b*f/(ul-ur);  视差=ul-ur;
    ...
    // 观测不到Map中的3D点,第i特征点与Map中点有对应
    std::vector<bool> mvbOutlier;   //主要用在Optimizer
    ...
    // 用于确定画格子时的边界
    static float mnMinX;  //图像矫正之后的横坐标左边界
    static float mnMaxX;   //横坐标右边界
    static float mnMinY;
    static float mnMaxY;
