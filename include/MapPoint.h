...
public:
    ...
    // a 已经和当前帧经过匹配（TrackReferenceKeyFrame，TrackWithMotionModel）但在优化过程中认为是外点  Tracking 969
    // b 已经和当前帧经过匹配且为内点，这类点也不需要再进行投影(投影的目的是什么?SearchByProjection（）由此增加当前帧的MapPoints匹配) Tracking 1485
    // c 不在当前相机视野中的点（即未通过isInFrustum判断）Frame.cpp
    bool mbTrackInView;
    ...
    long unsigned int mnCorrectedByKF;  //表示该点被当前关键帧闭环修正过
    long unsigned int mnCorrectedReference;
    cv::Mat mPosGBA;  //全局BA后MapPoint的位姿
    long unsigned int mnBAGlobalForKF;  //BA时，全局关键帧数
