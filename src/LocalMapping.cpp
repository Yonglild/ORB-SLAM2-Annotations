...
void LocalMapping::Run()
{
    ...
    while(1)
    {
        ...
        if(CheckNewKeyFrames())
        {
            ...
            {
                // VI-D Local BA
                if(mpMap->KeyFramesInMap()>2)  //地图中关键帧个数>2,则局部ＢＡ（进行和当前帧（最后一帧）相连的关键帧及MapPoint做局部BA优化）
        ...
        ResetIfRequested();   //清除mlpRecentAddedMapPoints

        // Tracking will see that Local Mapping is not busy
        SetAcceptKeyFrames(true);   //允许从Ｔracking接受关键帧
...
/**Tracking.cpp  653   Tracking 与 LocalMapping 的桥梁
 * @brief 插入关键帧
 *
...
/**
 * @brief 处理列表中的关键帧,得到当前关键帧生成的点(待检)
 * 
...
void LocalMapping::ProcessNewKeyFrame()
{
    ...
    const vector<MapPoint*> vpMapPointMatches = mpCurrentKeyFrame->GetMapPointMatches();//获取该关键帧的MapPoints
    ...
    for(size_t i=0; i<vpMapPointMatches.size(); i++)
    {
        ...
        if(pMP)
        {
            if(!pMP->isBad())
            {
                // 非当前帧生成的MapPoints,（在TrackLocalMap函数中将局部地图中的MapPoints与当前帧进行了匹配， 但没有对这些匹配上的MapPoints与当前帧进行关联）
				// 为当前帧在tracking过程跟踪到的MapPoints更新属性
				// KF与pMP有连接，但pMP与ＫＦ没连接，故要连接（为什么会导致这样？？？）TrackLocalMap 1177
...
void LocalMapping::MapPointCulling()
{
    ...
    while(lit!=mlpRecentAddedMapPoints.end())
    {
        ...
        //GetFoundRatio低表示该地图点在很多关键帧的视野内，但是没有匹配上很多的特征点
        ...
        else if(((int)nCurrentKFid-(int)pMP->mnFirstKFid)>=3)    //要不要再加一个条件？且观测次数>=3
            // 步骤4：从建立该点开始，已经过了3个关键帧，则认为是稳定的质量高的点
...
void LocalMapping::CreateNewMapPoints()
{
    ...
    if(mbMonocular)
        nn=20;
    // 步骤1：在当前关键帧的共视关键帧中找到共视程度最高的20帧相邻帧vpNeighKFs. * 权值是否要大于15？？？
    const vector<KeyFrame*> vpNeighKFs = mpCurrentKeyFrame->GetBestCovisibilityKeyFrames(nn);

    ORBmatcher matcher(0.6,false);

    //这得到当前关键帧在世界坐标系的坐标吗？？？与ow1的区别？
    //计算重投影误差要用
    ...
    for(size_t i=0; i<vpNeighKFs.size(); i++)
    {
        ...
        vector<pair<size_t,size_t> > vMatchedIndices;  //1  索引  2.索引
...
void LocalMapping::SearchInNeighbors()
{
    ...
    //遍历共识程度最高的前nn帧以及相应的5帧，都存入vpTargetKFs
    for(vector<KeyFrame*>::const_iterator vit=vpNeighKFs.begin(), vend=vpNeighKFs.end(); vit!=vend; vit++)
    {
        ...
        // Extend to some second neighbors 遍历该帧共识程度最好的5帧
    ...
    for(vector<KeyFrame*>::iterator vitKF=vpTargetKFs.begin(), vendKF=vpTargetKFs.end(); vitKF!=vendKF; vitKF++)
    {
        ...
        // 当前一级邻接和二级邻接关键帧中所有的MapPoints加入vpFuseCandidates
    ...
    // 更新covisibility图
    mpCurrentKeyFrame->UpdateConnections();//mConnectedKeyFrameWeights，mvpOrderedConnectedKeyFrames，mvOrderedWeights
}
...
void LocalMapping::KeyFrameCulling()
{
    ...
    // 步骤1：根据Covisibility Graph提取当前帧的共视关键帧；共识点数大于15的才能成为共识关键帧？？
        ...
        for(size_t i=0, iend=vpMapPoints.size(); i<iend; i++)
        {
            ...
            if(pMP)
            {
                if(!pMP->isBad())
                {
                    if(!mbMonocular)
                    {
                        // 对于双目或ＲＧＢＤ，仅考虑近处的MapPoints，不超过mbf * 35 / fx
                        if(pKF->mvDepth[i]>pKF->mThDepth || pKF->mvDepth[i]<0)
                            continue;
                    }

                    nMPs++;
                    // 粗筛：MapPoints至少被三个关键帧观测到（未有尺度约束要求）
                    if(pMP->Observations()>thObs)
                    {
                        const int &scaleLevel = pKF->mvKeysUn[i].octave;
                        const map<KeyFrame*, size_t> observations = pMP->GetObservations();//关键帧与索引
                        int nObs=0;
                        ...
                        // 精筛：该MapPoint至少被三个关键帧观测到（有尺度约束要求）
                        if(nObs>=thObs)
                        {
