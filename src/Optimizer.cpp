                                 ...
                                 int nIterations, bool* pbStopFlag, const unsigned long nLoopKF, const bool bRobust)
{
    ...
    vbNotIncludedMP.resize(vpMP.size());   //？？？什么意思
    ...
    for(size_t i=0; i<vpMP.size(); i++)
    {
        ...
        if(nEdges==0)   //该顶点没有对应的边
        {
            optimizer.removeVertex(vPoint);
            vbNotIncludedMP[i]=true;     //标记，该顶点不在优化范围
        }
        else
        {
...
/**Tracking 有3个地方用到 969 1135恒速模型跟踪 1193
 * @brief Pose Only Optimization(3D-2D)
 * 
 * 3D-2D 最小化重投影误差 e = (u,v) - project(Tcw*Pw) \n
...
int Optimizer::PoseOptimization(Frame *pFrame)
{
    ...
    // 步骤1：构造g2o优化器
    g2o::SparseOptimizer optimizer;
    g2o::BlockSolver_6_3::LinearSolverType * linearSolver;  //定义线性求解器实例
    ...
    for(int i=0; i<N; i++)
    {
        ...
        if(pMP)
        {
            ...
            // 单目情况和RGBD, 也有可能在双目下, 当前帧的左兴趣点找不到匹配的右兴趣点
            if(pFrame->mvuRight[i]<0)
            {
                nInitialCorrespondences++;   //参与优化的点数++
                ...
                g2o::EdgeSE3ProjectXYZOnlyPose* e = new g2o::EdgeSE3ProjectXYZOnlyPose();   //创建一条新边

                e->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex*>(optimizer.vertex(0)));  //连接1个顶点
                e->setMeasurement(obs);    //添加观测值
                const float invSigma2 = pFrame->mvInvLevelSigma2[kpUn.octave];
                e->setInformation(Eigen::Matrix2d::Identity()*invSigma2);  //信息矩阵（协防差矩阵的逆）
            ...
            else  // Stereo observation 双目
            {
                ...
                const cv::KeyPoint &kpUn = pFrame->mvKeysUn[i];
                const float &kp_ur = pFrame->mvuRight[i];  // 对于双目，mvuRight存储了左目像素点在右目中的对应点的横坐标
    ...
    for(size_t it=0; it<4; it++)
    {
        ...
        optimizer.optimize(its[it]);   //迭代10次
        ...
        for(size_t i=0, iend=vpEdgesMono.size(); i<iend; i++)
        {
            ...
            const size_t idx = vnIndexEdgeMono[i];   //第i条边
            ...
            {                
                ...
                e->setLevel(1);                 // 设置为outlier，不参与下一轮
                nBad++;
            }
            else
            {
...
void Optimizer::LocalBundleAdjustment(KeyFrame *pKF, bool* pbStopFlag, Map* pMap)
{
    ...
    // 步骤4：得到能被局部MapPoints观测到，但不属于局部关键帧的关键帧，这些关键帧在局部BA优化时不优化（lFixedCameras）
    ...
    // 步骤6：添加顶点：Pose of Local KeyFrame(相机位姿)
    for(list<KeyFrame*>::iterator lit=lLocalKeyFrames.begin(), lend=lLocalKeyFrames.end(); lit!=lend; lit++)
    {
        KeyFrame* pKFi = *lit;
        g2o::VertexSE3Expmap * vSE3 = new g2o::VertexSE3Expmap();
        ...
        vSE3->setFixed(pKFi->mnId==0);//id=0则为ture;第一帧位置固定
        optimizer.addVertex(vSE3);
    ...
    //单目
    vector<g2o::EdgeSE3ProjectXYZ*> vpEdgesMono;   //边的集合
    vpEdgesMono.reserve(nExpectedSize);

    vector<KeyFrame*> vpEdgeKFMono;  //关键帧的集合
    vpEdgeKFMono.reserve(nExpectedSize);

    vector<MapPoint*> vpMapPointEdgeMono;    //地图点的集合
    ...
    for(list<MapPoint*>::iterator lit=lLocalMapPoints.begin(), lend=lLocalMapPoints.end(); lit!=lend; lit++)
    {
        ...
        for(map<KeyFrame*,size_t>::const_iterator mit=observations.begin(), mend=observations.end(); mit!=mend; mit++)
        {
            ...
            if(!pKFi->isBad())
            {                
                const cv::KeyPoint &kpUn = pKFi->mvKeysUn[mit->second];   //特征点的像素坐标（u,v）
                ...
                if(pKFi->mvuRight[mit->second]<0)
                {
                    ...
                    optimizer.addEdge(e);
                    vpEdgesMono.push_back(e);   //参与优化的边集合
                    vpEdgeKFMono.push_back(pKFi);  //参与优化的关键帧集合
                    vpMapPointEdgeMono.push_back(pMP);  //参与优化的点集合
                }
    ...
    for(size_t i=0, iend=vpEdgesMono.size(); i<iend;i++)
    {
        ...
        if(e->chi2()>5.991 || !e->isDepthPositive())
        {
            ...
            vToErase.push_back(make_pair(pKFi,pMP));   //该边涉及到的点和帧
        }
    }
    ...
    if(!vToErase.empty())
    {
        for(size_t i=0;i<vToErase.size();i++)
        {
            ...
            pKFi->EraseMapPointMatch(pMPi);  //删除彼此之间的联系
                                       ...
                                       const map<KeyFrame *, set<KeyFrame *> > &LoopConnections, const bool &bFixScale)
{
    ...
    // Set KeyFrame vertices
    // 步骤2：将地图中所有keyframe的位姿作为顶点添加到优化器
    // 尽可能使用经过Sim3调整的位姿
    for(size_t i=0, iend=vpKFs.size(); i<iend;i++)
    {
        ...
        //pKF的KeyFrameAndPose(帧与位姿)
        LoopClosing::KeyFrameAndPose::const_iterator it = CorrectedSim3.find(pKF);

        //顶点的初始值赋予（如果该关键帧在闭环时通过Sim3传播调整过，用校正后的位姿）
        if(it!=CorrectedSim3.end())
        {
            vScw[nIDi] = it->second;   //vScw：存放地图中中所有关键帧的位姿(nIDi=id)（用调整过的位姿）
            VSim3->setEstimate(it->second);  //顶点的初始值
        }
    ...
    // Set Loop edges  （回环处相连帧的边，没有加闭环帧的关系）
    //LoopConnections：当前关键帧的一级相连帧pKFi的相连帧集合
    // 步骤3：添加边：LoopConnections是闭环时因为MapPoints调整而出现的新关键帧连接关系（不是当前帧与闭环匹配帧之间的连接关系）
    for(map<KeyFrame *, set<KeyFrame *> >::const_iterator mit = LoopConnections.begin(), mend=LoopConnections.end(); mit!=mend; mit++)
    {
        KeyFrame* pKF = mit->first;   //一级帧pKF
        const long unsigned int nIDi = pKF->mnId;
        const set<KeyFrame*> &spConnections = mit->second;   //一级帧pKF的相连帧集合（二级帧）
        const g2o::Sim3 Siw = vScw[nIDi];
        const g2o::Sim3 Swi = Siw.inverse();

        for(set<KeyFrame*>::const_iterator sit=spConnections.begin(), send=spConnections.end(); sit!=send; sit++)
        {
            const long unsigned int nIDj = (*sit)->mnId;   //二级帧id
            if((nIDi!=pCurKF->mnId || nIDj!=pLoopKF->mnId) && pKF->GetWeight(*sit)<minFeat)
                continue;

            const g2o::Sim3 Sjw = vScw[nIDj];
            // 得到两个pose间的Sim3变换（二级帧j-->一级帧i）
    ...
    // Set normal edges（父子帧之间的边，第i帧与其闭环匹配帧之间的连接关系，共识帧之前的边)
    ...
    for(size_t i=0, iend=vpKFs.size(); i<iend; i++)
    {
        ...
        if(pParentKF)
        {
            ...
            if(itj!=NonCorrectedSim3.end())
                ...
                Sjw = vScw[nIDj];   //父帧 j 的位姿

            g2o::Sim3 Sji = Sjw * Swi;  //父子帧相对位姿位姿
    ...
    // 步骤7：步骤5和步骤6优化得到关键帧的位姿后，MapPoints根据参考帧优化前后的相对关系映射调整自己的位置
