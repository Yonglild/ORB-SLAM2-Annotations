...
void Tracking::Track() {
        ...
        if (mState == NOT_INITIALIZED) {
            if (mSensor == System::STEREO || mSensor == System::RGBD)
                StereoInitialization();    //如何初始化???
            else
                MonocularInitialization();

            mpFrameDrawer->Update(this);   ////将跟踪线程的数据拷贝到绘图线程（图像、特征点、地图、跟踪状态）

            if (mState != OK)
                return;
        } else// 步骤2：跟踪,bOK判断整个跟踪成功与否
        {
            // System is initialized. Track Frame.
            // bOK为临时变量，用于表示每个函数是否执行成功;一直到最后决定mState
            bool bOK;
            ...
            if (!mbOnlyTracking)   //跟踪定位+地图更新
            {
                ...
                if (mState == OK) {
                    ...
                    // 更新Fuse函数和SearchAndFuse函数替换的MapPoints(LocalMapping::SearchInNeighbors()融合)
                    ...
                    if (mVelocity.empty() || mCurrentFrame.mnId < mnLastRelocFrameId + 2) {
                        /* 将上一帧的位姿作为当前帧的初始位姿
                         通过BoW的方式在参考帧中找当前帧特征点的匹配点MapPoint
                        ...
                        if (!bOK)
                            // TrackReferenceKeyFrame是跟踪参考帧，不能根据固定运动速度模型预测当前帧的位姿态，只通过bow加速匹配（SearchByBow）
                            // 最后通过优化得到优化后的位姿
                            bOK = TrackReferenceKeyFrame();
                    }
                } else {
            ...
            {     // 只进行跟踪tracking，局部地图不工作（无地图更新）!!!
                ...
                } else   //tracking正常
                {
                    ...
                    // mbVO为true表明此帧跟踪了很少的MapPoints，少于10个，要跪的节奏   1163 TrackMotionMode
                    if (!mbVO) {
                        // In last frame we tracked enough MapPoints in the map
                        // mbVO为0则表明此帧跟踪了(匹配上了)很多的3D map点，非常好
                        if (!mVelocity.empty()) {
                            bOK = TrackWithMotionModel();
                            // 这个地方是不是应该加上：
                            // if(!bOK)
                    ...
                    } else  // 跟踪正常(没有跟丢)，但mbVO为1，则表明此帧匹配了很少的3D map点，少于10个，要跪的节奏，既做跟踪又做重定位
                    {
                        ...
                        bool bOKMM = false;    //是否跟踪成功
                        bool bOKReloc = false;   //是否重定位成功
                        vector<MapPoint *> vpMPsMM;  //每一个数组元素存的都是指针
                        vector<bool> vbOutMM;
                        cv::Mat TcwMM;

                        if (!mVelocity.empty())     //既做跟踪
                        {
            ...
            // 步骤2.2：在帧间匹配(Frame--Frame)得到初始的姿态后，现在对local map进行跟踪得到更多的匹配，并优化当前位姿
            ...
            if (bOK) {
                ...
                if (!mLastFrame.mTcw.empty()) {
                    ...
                    cv::Mat LastTwc = cv::Mat::eye(4, 4, CV_32F);  //对角阵
                    mLastFrame.GetRotationInverse().copyTo(LastTwc.rowRange(0, 3).colRange(0, 3));
                    mLastFrame.GetCameraCenter().copyTo(LastTwc.rowRange(0, 3).col(3));
                    mVelocity = mCurrentFrame.mTcw * LastTwc; // Tcl
                } else
                    mVelocity = cv::Mat();   //若上一贞速度为空，则令速度也为空

                mpMapDrawer->SetCurrentCameraPose(mCurrentFrame.mTcw);

                // Clean VO matches
                // 步骤2.4:清楚当前帧中未被跟踪的点(即点的被关键帧观测次数<1,其他关键帧也没见过的点),留下质量较高的点
                for (int i = 0; i < mCurrentFrame.N; i++){
            ...
            // Delete temporal MapPoints    1071
        ...
        if(mState==LOST)
        {
            if(mpMap->KeyFramesInMap()<=5)
            {
                ...
                mpSystem->Reset();   //如果系统开始没多久就失败了，就Reset(重置).
                return;   //结束当前程序块
            }
            //否则，整个Track()会一直重定位，直到跟踪成功
        }
    ...
    {
        // 计算相对姿态T_currentFrame_reference
        cv::Mat Tcr = mCurrentFrame.mTcw*mCurrentFrame.mpReferenceKF->GetPoseInverse();  //当前帧相对于当前参考帧(一般为最近的参考帧)的位姿
        mlRelativeFramePoses.push_back(Tcr);
        mlpReferences.push_back(mpReferenceKF);
        mlFrameTimes.push_back(mCurrentFrame.mTimeStamp);  //当前帧时间戳
...
{
    if(mCurrentFrame.N>500)  //特征点>500
    {
        // Set Frame pose to the origin
        // 步骤1：设置相机姿态，即设置相机坐标系到世界坐标系的变换矩阵，世界坐标系到相机坐标系的变换矩阵
        ...
        for(int i=0; i<mCurrentFrame.N;i++)
        {
            ...
            if(z>0)
            {
                ...
                // a.表示该MapPoint可以被初始KeyFrame的哪个特征点观测到
                pNewMP->AddObservation(pKFini,i);   //map<KeyFrame* ,size_t> mObservations
                ...
                pKFini->AddMapPoint(pNewMP,i);    //mvpMapPoints[idx]=pMP;

                // 步骤4.6：将该MapPoint添加到当前帧的mvpMapPoints中
                // 为当前Frame(不是关键帧)的特征点与MapPoint之间建立索引
        ...
        // 步骤4：往LocalMapping添加该初始关键帧(Tracking--->LocalMapping)
        ...
        mpMap->mvpKeyFrameOrigins.push_back(pKFini);    //Loopclosing.cpp 全局BA用到
...
void Tracking::MonocularInitialization()
{
    ...
    if(!mpInitializer)
    {
        ...
        if(mCurrentFrame.mvKeys.size()>100)
        {
            ...
            // 记录最近的一帧
            mLastFrame = Frame(mCurrentFrame);
            // mvbPrevMatched为前一帧的特征点。最大的情况就是所有特征点都被跟踪上
    ...
    else
    {
        ...
        // 步骤5：开启了两个线程，求解H和F模型。通过H模型或F模型进行单目初始化，得到两帧间相对运动、初始MapPoints
...
/**
 * @brief 检查上一帧中的MapPoints是否被替换,更新mLastFrame的点
 * 
 * Local Mapping线程可能会将关键帧中某些MapPoints进行替换，由于tracking中需要用到mLastFrame，这里检查并更新上一帧中被替换的MapPoints
 * @see LocalMapping::SearchInNeighbors(),检查并融合当前关键帧与相邻帧（两级相邻）重复的MapPoints
 * 或者LoopClosing在闭环过程中也有可能替换
 */
void Tracking::CheckReplacedInLastFrame()
{
    for(int i =0; i<mLastFrame.N; i++)
    {
        ...
        if(pMP)
        {
            MapPoint* pRep = pMP->GetReplaced();  //返回替换后的点
            if(pRep)
            {
 ...
 */
bool Tracking::TrackReferenceKeyFrame()    //将最近的参考关键帧作为当前位姿的初始值进行优化,确认是否值得跟踪
{
    ...
    // 步骤2：通过特征点的BoW加快当前帧与Tracking参考帧之间的特征点匹配
    // vpMapPointMatches:得到更新当前帧中MapPoints对应的匹配(索引与3D点)
    ...
    // 步骤4:已知初始值,通过优化3D-2D的重投影误差来获得位姿??????(了解优化过程)
    ...
    for(int i =0; i<mCurrentFrame.N; i++)
    {
        ...
        {
            if(mCurrentFrame.mvbOutlier[i])   //优化后,第i特征点与Map中3D点没有对应的,外点
            {
                MapPoint* pMP = mCurrentFrame.mvpMapPoints[i];

                mCurrentFrame.mvpMapPoints[i]=static_cast<MapPoint*>(NULL);
                mCurrentFrame.mvbOutlier[i]=false;
                pMP->mbTrackInView = false;   //认为是外点,不需要投影
...
/**
 * @brief 双目或rgbd摄像头根据深度值为上一帧临时产生新的MapPoints(标记一下,后面会删除)
 *
...
void Tracking::UpdateLastFrame()
{
    ...
    cv::Mat Tlr = mlRelativeFramePoses.back();   //上一针相对于当前参考帧的位姿

    mLastFrame.SetPose(Tlr*pRef->GetPose()); // Tlr*Trw = Tlw 1:last r:reference w:world

    // 如果上一帧为关键帧(不需要更新)，或者单目的情况，则退出
    ...
    // 步骤2.1：得到上一帧有深度值的特征点
    vector<pair<float,int> > vDepthIdx;   //1.深度 2.索引
    ...
    //条件:1.第i特征点没有对应MapPoint;2.所对应的MapPoint没被其它关键帧看到(认为是不好的点);
    int nPoints = 0;
    for(size_t j=0; j<vDepthIdx.size();j++)
    {
        ...
        if(!pMP)
            bCreateNew = true;
        else if(pMP->Observations()<1)  //该地图点不是没有被其他关键帧看见过；即质量不高
        {
            bCreateNew = true;
        }
        ...
        if(bCreateNew)
        {
            ...
            mLastFrame.mvpMapPoints[i]=pNewMP; // 添加新的MapPoint(更新,但后面还会删除)
...
/**
 * @brief 根据匀速度模型对上一帧的MapPoints(有临时增加的)进行跟踪
 * 
...
bool Tracking::TrackWithMotionModel()
{
    ...
    // 根据Const Velocity Model初始化当前帧的位姿
    ...
    // 步骤2：根据匀速度模型进行对上一帧的MapPoints进行投影跟踪,获取更新当前帧的3D点
    // 根据上一帧特征点对应的3D点投影的位置缩小特征点匹配范围,得到匹配点数
 ...
 * 2. 对局部MapPoints进行投影匹配,由此增加当前帧的MapPoints
 * 3. 根据匹配对(3D-2D PnP)估计当前帧的姿态
 * 4. 根据姿态剔除误匹配
...
bool Tracking::TrackLocalMap()
{
    ...
    // 步骤1：更新局部地图：局部关键帧mvpLocalKeyFrames和局部地图点mvpLocalMapPoints
    UpdateLocalMap();  //根据当前帧的MapPoint更新局部地图

    // 步骤2：在局部地图中投影查找与当前帧匹配的MapPoints,由此增加当前帧的MapPoints
    SearchLocalPoints();

    // Optimize Pose
    // Relocalization、TrackReferenceKeyFrame、TrackWithMotionModel中也有pnp位姿优化.
    ...
    for(int i=0; i<mCurrentFrame.N; i++)
    {
        if(mCurrentFrame.mvpMapPoints[i])
        {
            ...
            if(!mCurrentFrame.mvbOutlier[i])
            {
                ...
                if(!mbOnlyTracking)
                {
                    // 该MapPoint被其它关键帧观测到过(即跟踪!!!!已有的MapPoint点被你跟踪到了)
    ...
    if(mnMatchesInliers<30)   //跟踪到的MapPoint<30;
        return false;
    else
        return true;
}
...
bool Tracking::NeedNewKeyFrame()
{
    ...
    if(nKFs<=2)
        nMinObs=2;
    int nRefMatches = mpReferenceKF->TrackedMapPoints(nMinObs);   //得到Tracking参考帧中跟踪到的MapPoint点(跟踪到的点必须是质量较高的点)
    ...
    if(mSensor!=System::MONOCULAR)// 双目或rgbd
    {
        for(int i =0; i<mCurrentFrame.N; i++)
        {
            if(mCurrentFrame.mvDepth[i]>0 && mCurrentFrame.mvDepth[i]<mThDepth)
            {
                ...
                if(mCurrentFrame.mvpMapPoints[i])
                    if(mCurrentFrame.mvpMapPoints[i]->Observations()>0)
                        nMap++;// 当前帧中被其他关键帧观测到的mappoints数，即观测到地图中的MapPoints数量,说明该点被当前帧跟踪了
            }
        }
    }
    else
    ...
    if((c1a||c1b||c1c)&&c2)
    {
        ...
        {
            mpLocalMapper->InterruptBA();  //打断LocalMapping,因为跟踪不行了
            if(mSensor!=System::MONOCULAR)
            {
                // 队列里不能阻塞太多关键帧
...
/**
 * @brief 将当前帧构造成关键帧,设置Tracking和当前帧的参考关键帧,在原来基础上创建新的MapPoint,送入Localmapping
 *
...
void Tracking::CreateNewKeyFrame()
{
    ...
    // 步骤2：将当前关键帧设置为Tracking参考关键帧以及当前帧的参考帧
    // 在UpdateLocalKeyFrames函数中会将与当前关键帧共视程度最高的关键帧设定为当前帧的参考关键帧
    mpReferenceKF = pKF;
    ...
    if(mSensor!=System::MONOCULAR)
    {
        ...
        if(!vDepthIdx.empty())
        {
            ...
            for(size_t j=0; j<vDepthIdx.size();j++)
            {
                ...
                if(!pMP)
                    bCreateNew = true;
                else if(pMP->Observations()<1)  //所对应的特征点没有被其他关键帧看到(不好的点)
                {
                    bCreateNew = true;
                    mCurrentFrame.mvpMapPoints[i] = static_cast<MapPoint*>(NULL);
                }

                if(bCreateNew)  //将不合格的点重新生成
                {
                    cv::Mat x3D = mCurrentFrame.UnprojectStereo(i);
...
/**
 * @brief 由此增加当前帧与局部地图的MapPoints匹配数
 * 
...
void Tracking::SearchLocalPoints()
{
    ...
    // 步骤2：将当前帧的局部MapPoints投影到当前帧，判断是否在视野范围内，然后进行投影匹配
    for(vector<MapPoint*>::iterator vit=mvpLocalMapPoints.begin(), vend=mvpLocalMapPoints.end(); vit!=vend; vit++)
    {
        ...
        // 步骤2.1：判断LocalMapPoints中的点是否在在视野内(5个条件)
        if(mCurrentFrame.isInFrustum(pMP,0.5))
        {
        	// 观测到该点的帧数加1，该MapPoint在当前帧的视野范围内
            pMP->IncreaseVisible();
    ...
    if(nToMatch>0)
    {
        ...
        // 步骤2.2：对视野范围内的MapPoints通过投影进行特征点匹配,由此增加当前帧的MapPoints(当前帧自己生成的MapPonit,则不需要投影)
...
{
    // This is for visualization  用于画图ＭapDrawer.cpp 43
...
/**清空原先的局部关键点,将局部关键帧mvpLocalKeyFrames的MapPoint加到局部关键点.
...
/**
 * @brief 根据当前帧的MapPoint更新局部关键帧与2个参考关键帧，called by UpdateLocalMap()
 *
...
void Tracking::UpdateLocalKeyFrames()
{
    ...
    // 步骤1：遍历当前帧的MapPoints，记录所有能观测到当前帧MapPoints的关键帧以及次数
    ...
    for(int i=0; i<mCurrentFrame.N; i++)
    {
        if(mCurrentFrame.mvpMapPoints[i])
        {
            ...
            if(!pMP->isBad())
            {
                // 遍历所有关键帧，能观测到当前帧MapPoints的关键帧的次数++
    ...
    // 策略2：与策略1得到的共识关键帧共视程度很高的关键帧作为局部关键帧
    for(vector<KeyFrame*>::const_iterator itKF=mvpLocalKeyFrames.begin(), itEndKF=mvpLocalKeyFrames.end(); itKF!=itEndKF; itKF++)
    {
    ...
    // 步骤3：更新Tracking参考关键帧与当前帧的参考关键帧，与自己共视程度最高的关键帧作为参考关键帧
    if(pKFmax)
    {
...
bool Tracking::Relocalization()
{
    ...
    // 步骤2：找到与当前帧相似的候选关键帧(有共同单词,且相似度高的)
    ...
    for(int i=0; i<nKFs; i++)
    {
        ...
        {
            // 步骤3：通过BoW进行匹配,vvpMapPointMatches[i]为当前帧的匹配的MapPoint
        ...
        for(int i=0; i<nKFs; i++)
        {
            ...
            if(!Tcw.empty())
            {
                ...
                for(int j=0; j<np; j++)
                {
                    ...
                    else  //不然,该内点没有对应的mappoint
                        mCurrentFrame.mvpMapPoints[j]=NULL;
                }

                // 步骤5：(当前帧3D)--2D 通过PoseOptimization对姿态进行优化求解，返回内点数（pMP）
