...
MapDrawer::MapDrawer(Map* pMap, const string &strSettingPath):mpMap(pMap)
{
    ...
    mKeyFrameSize = fSettings["Viewer.KeyFrameSize"]; //宽度占总宽度比例为0.05
    mKeyFrameLineWidth = fSettings["Viewer.KeyFrameLineWidth"]; //设置绘制图形时线的宽度1
    mGraphLineWidth = fSettings["Viewer.GraphLineWidth"];//0.9
    mPointSize = fSettings["Viewer.PointSize"];//点的直径 2
...
void MapDrawer::DrawKeyFrames(const bool bDrawKF, const bool bDrawGraph)
{
    ...
    if(bDrawGraph)
    {
        ...
        glBegin(GL_LINES);
        //遍历所有关键帧，找出每一个关键帧共识程度较高的关键帧vCovKFs(局部变量),
        for(size_t i=0; i<vpKFs.size(); i++)
        {
            // Covisibility Graph
