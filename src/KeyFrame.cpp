    ...
    mbToBeErased(false), mbBad(false), mHalfBaseline(F.mb/2), mpMap(pMap)
{
    ...
    mGrid.resize(mnGridCols);  //mGrid为什么是三个vector嵌套
    for(int i=0; i<mnGridCols;i++)
    {
        mGrid[i].resize(mnGridRows);
        for(int j=0; j<mnGridRows; j++)
            mGrid[i][j] = F.mGrid[i][j];  //每个格子分配的特征点数
    }
...
/**
 * @brief 为关键帧之间添加连接
 * std::map<KeyFrame*,int> 与该关键帧连接的关键帧与权重!!!!
 * 更新了mConnectedKeyFrameWeights
 * @param pKF    关键帧
 * @param weight 权重，该关键帧与pKF共同观测到的3d点数量（共识点数）
 */
...
{
    {
        ...
        else if(mConnectedKeyFrameWeights[pKF]!=weight) // 之前连接的权重不一样,则更新
            mConnectedKeyFrameWeights[pKF]=weight;
        else
            return;
    }
...
/**
 * @brief 更新，按照权重对连接的关键帧进行排序
 * mConnectedKeyFrameWeights-->vPairs
...
void KeyFrame::UpdateBestCovisibles()
{
    ...
    vector<pair<int,KeyFrame*> > vPairs;   //first is weight,second is pkf
    ...
    {
        lKFs.push_front(vPairs[i].second);  //大的放前面
 ...
 * @return 连接的关键帧
 */
set<KeyFrame*> KeyFrame::GetConnectedKeyFrames()   //set用法是什么？？？
{
    unique_lock<mutex> lock(mMutexConnections);
    set<KeyFrame*> s;
...
/**    在UpdateBestCovisibles()中得到
 * @brief 返回与该关键帧连接的关键帧(已按权值排序)
 * @return 连接的关键帧
 */
...
/** mvpMapPoints是在KeyFrame类中protected类型？？
...
//根据索引删除关键帧里对应的MapPoint
void KeyFrame::EraseMapPointMatch(const size_t &idx)
{
    unique_lock<mutex> lock(mMutexFeatures);
    mvpMapPoints[idx]=static_cast<MapPoint*>(NULL);
}
//根据在关键帧上的点找到索引，删除对应的MapPoint点
void KeyFrame::EraseMapPointMatch(MapPoint* pMP)
{
    int idx = pMP->GetIndexInKeyFrame(this);    //mObservations std::map<KeyFrame*,size_t>
    if(idx>=0)
        mvpMapPoints[idx]=static_cast<MapPoint*>(NULL);
}

//用pMP替代原先的MapPoint.为什么要替代，什么时候替代？
//闭环的时候，会更新KeyFrame与MapPoint之间的关系
void KeyFrame::ReplaceMapPointMatch(const size_t &idx, MapPoint* pMP)
{
    mvpMapPoints[idx]=pMP;
}

//将mvpMapPoints中的点放进s中，返回s
...
int KeyFrame::TrackedMapPoints(const int &minObs)
{
    ...
    for(int i=0; i<N; i++)  //遍历该关键帧上的所有关键点
    {
        MapPoint* pMP = mvpMapPoints[i];
        if(pMP)
        {
...
/**LoopClosing.cpp 524 667
 * @brief 更新图的连接(关键帧和关键帧之间的边)
 * 
...
void KeyFrame::UpdateConnections()
{
    ...
    {   //遍历该关键帧上的所有MapPoint,统计在该点上共识的关键帧，并存在KFcounter
        MapPoint* pMP = *vit;
        ...
        // 对于每一个MapPoint点，observations记录了可以观测到该MapPoint的所有关键帧和索引
        ...
        for(map<KeyFrame*,size_t>::iterator mit=observations.begin(), mend=observations.end(); mit!=mend; mit++)
        {
            ...
            if(mit->first->mnId==mnId)
                continue;
            KFcounter[mit->first]++;   //当前关键帧与另一关键帧的共识MapPoint点数++
        }
    }
    ...
    // 筛选：vPairs记录与其它关键帧共视点数大于th的关键帧
    // pair<int,KeyFrame*>将关键帧的权重写在前面，关键帧写在后面方便后面排序
    vector<pair<int,KeyFrame*> > vPairs;
    vPairs.reserve(KFcounter.size());
    //遍历KFcounter上所有关键帧，找到对应权重最大的关键帧（共视程度最高的关键帧）
    for(map<KeyFrame*,int>::iterator mit=KFcounter.begin(), mend=KFcounter.end(); mit!=mend; mit++)
    {
        ...
        if(mit->second>=th)
        {
            ...
            vPairs.push_back(make_pair(mit->second,mit->first));
            // 更新KFcounter中该关键帧的mConnectedKeyFrameWeights，更新该关键帧与当前帧的连接权重，并更新该关键帧的连接关系
    ...
    // vPairs里存的是与当前关键帧相互共视程度比较高的关键帧和共视权重，由大到小
    sort(vPairs.begin(),vPairs.end());
    list<KeyFrame*> lKFs;
    list<int> lWs;
    ...
    //===============3==================================
    {
        ...
        mvpOrderedConnectedKeyFrames = vector<KeyFrame*>(lKFs.begin(),lKFs.end());   //都是与当前关键帧共识程度高的关键帧
        mvOrderedWeights = vector<int>(lWs.begin(), lWs.end());
        ...
        if(mbFirstConnection && mnId!=0)
        {
            ...
            // 建立双向连接关系
            mpParent->AddChild(this);  //this为当前关键帧？？
            mbFirstConnection = false;
        }
    }
}

//添加子关键帧（说明该关键帧是各个子关键帧的共识程度最高的帧）
void KeyFrame::AddChild(KeyFrame *pKF)
{
    unique_lock<mutex> lockCon(mMutexConnections);
    mspChildrens.insert(pKF);    //std::set是集合，相比vector，进行插入数据这样的操作时会自动排序
}
...
//删除该帧
// ＬoopClosing.cpp 127 如果距离上次闭环没多久（小于10帧），或者map中关键帧总共还没有10帧，则不进行闭环检测。并将该帧存入mpKeyFrameDB,后删除该帧
...
{   
    ...
    /****让别人删除与我的联系****/
    // 遍历所有与自己有链接的关键帧，让其它的KeyFrame删除与自己的联系
    for(map<KeyFrame*,int>::iterator mit = mConnectedKeyFrameWeights.begin(), mend=mConnectedKeyFrameWeights.end(); mit!=mend; mit++)
        mit->first->EraseConnection(this);
    //遍历该关键帧上所有的MapPoint,让点删除与自己的联系
    ...
    /****删除我与别人的联系****/
    {
        ...
        mvpOrderedConnectedKeyFrames.clear();        //排序后的关键帧（共识程度较高的）

        // Update Spanning Tree？？？
        set<KeyFrame*> sParentCandidates;  //父关键帧候选帧集合
        sParentCandidates.insert(mpParent); //将该关键帧的父关键帧（共识程度最高的)作为候选关键帧A

        // Assign at each iteration one children with a parent (the pair with highest covisibility weight)
        // Include that children as new parent candidate for the rest
        // 如果这个关键帧有自己的孩子关键帧，告诉这些子关键帧，它们的父关键帧不行了，赶紧找新的父关键帧（哈哈哈哈）
        while(!mspChildrens.empty())
        {
            ...
            // 遍历该关键帧的每一个子关键帧，让它们更新它们指向的父关键帧 C
            for(set<KeyFrame*>::iterator sit=mspChildrens.begin(), send=mspChildrens.end(); sit!=send; sit++)
            {
                ...
                vector<KeyFrame*> vpConnected = pKF->GetVectorCovisibleKeyFrames();  //即mvpOrderedConnectedKeyFrames
                // 遍历子关键帧的相连关键帧（共视关键帧）D，E，F，（A）
                for(size_t i=0, iend=vpConnected.size(); i<iend; i++)
                {
                    //遍历候选父关键帧 A
                    for(set<KeyFrame*>::iterator spcit=sParentCandidates.begin(), spcend=sParentCandidates.end(); spcit!=spcend; spcit++)
                    {
                    // 要删除帧的子节点和父节点（祖孙节点）之间存在连接关系（共视）才有可能更换成功！！
                    // 举例：B-->A（B的父节点是A） C-->B（C的父节点是B） D--C（D与C相连） E--C（E与C相连） F--C（F与C相连） D-->A（D的父节点是A） E-->A（E的父节点是A） (C-->A永力按)
        ...
        mTcp = Tcw*mpParent->GetPoseInverse();   //相对于父关键帧
        mbBad = true;
    }
...
bool KeyFrame::isBad()  //这个帧不存在
{
    unique_lock<mutex> lock(mMutexConnections);
    return mbBad;
}

//删除与pKF的连接,并更新(UpdateBestCovisibles中mvpOrderedConnectedKeyFrames为什么不用>15的共识关键帧？？？)
void KeyFrame::EraseConnection(KeyFrame* pKF)
{
    ...
    if(bUpdate)
        UpdateBestCovisibles();        //关键帧i删除与pKF关键帧的连接后，按照权重对连接的关键帧进行排序。
}

// r为边长（半径）  去畸变的点和未去的点作比较，筛选出来
// 那里用到？？？？
vector<size_t> KeyFrame::GetFeaturesInArea(const float &x, const float &y, const float &r) const
{
    ...
    for(int ix = nMinCellX; ix<=nMaxCellX; ix++)
    {
        ...
        {
            const vector<size_t> vCell = mGrid[ix][iy];   //每个Grid[][]里有一组特征点
            for(size_t j=0, jend=vCell.size(); j<jend; j++)  //遍历这一组特征点
            {
                const cv::KeyPoint &kpUn = mvKeysUn[vCell[j]];   //vCell中第j个特征点
...
cv::Mat KeyFrame::UnprojectStereo(int i)
{
    ...
    if(z>0)
    {
        ...
        // mvDepth对应的校正前的特征点，因此这里对校正前特征点反投影！！！为什么不是校正后呢？？？？
