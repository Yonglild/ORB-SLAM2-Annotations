...
{
    mnCovisibilityConsistencyTh = 3;   //计数器阈值默认3 连续3组有连接
}
...
void LoopClosing::Run()
{
    ...
    while(1)
    {
        ...
        if(CheckNewKeyFrames())
        {
            ...
            if(DetectLoop())  //遍历与当前帧相似的关键帧作为闭环候选帧，经”子连续组“等工序作粗步筛选出闭环候选帧mvpEnoughConsistentCandidates。
            {
               // Compute similarity transformation [sR|t]
               // In the stereo/RGBD case s=1
               if(ComputeSim3())     //计算当前帧与所有闭环候选帧的Sim3变换,计算成功并检验,得到最终的闭环帧
               {
                   // Perform loop fusion and pose graph optimization
                   CorrectLoop();   //通过求解的Sim3以及相对姿态关系,闭环操作
               }
            }
        }
        ...
        //usleep(5000);
		std::this_thread::sleep_for(std::chrono::milliseconds(5));   //？？？睡觉5000秒？
	}
...
//遍历与当前帧相似的关键帧作为闭环候选帧，经”子连续组“等工序作粗步筛选出闭环候选帧mvpEnoughConsistentCandidates。
...
{
    {
        ...
        mpCurrentKF->SetNotErase();    //不应该擦除这个帧，防止在其他线程被擦除掉？
    }
    ...
    if(vpCandidateKFs.empty())
    {
        ...
        mpCurrentKF->SetErase();  //该关键帧没价值，实现不了闭环
        return false;
    }
    ...
    // ConsistentGroup.first对应每个“连续组”中的关键帧，ConsistentGroup.second为每个“连续组”的序号，计数器？
    vector<ConsistentGroup> vCurrentConsistentGroups;
    vector<bool> vbConsistentGroup(mvConsistentGroups.size(),false);    //？？？
    for(size_t i=0, iend=vpCandidateKFs.size(); i<iend; i++)   //遍历所有的闭环候选帧
    {
        KeyFrame* pCandidateKF = vpCandidateKFs[i];

        // 将自己以及与自己相连的关键帧构成一个“子候选组i”。
        ...
        {   //“子连续组iG” .注：“子连续组”由历史“子候选组”组成
            set<KeyFrame*> sPreviousGroup = mvConsistentGroups[iG].first;

            //检测”子候选组i“中是否与该“子连续组iG”有同样的帧
            ...
            if(bConsistent)  //若该“子候选组i”与“子连续组iG”有同样的帧，“子连续组iG”计数++，
            {
                int nPreviousConsistency = mvConsistentGroups[iG].second;
                int nCurrentConsistency = nPreviousConsistency + 1;
                if(!vbConsistentGroup[iG])// 这里作者原本意思是不是应该是vbConsistentGroup[i]而不是vbConsistentGroup[iG]呢？（wubo???）
                {
                    // 将该“子候选组i”打上编号加入到“当前连续组”
                    ConsistentGroup cg = make_pair(spCandidateGroup,nCurrentConsistency);
                    vCurrentConsistentGroups.push_back(cg);  //将与“子连续组”有联系的“子候选组”压入，为下文更新mvConsistentGroups
                    vbConsistentGroup[iG]=true; //this avoid to include the same group more than once 不懂？？
                }
                //说明所有“子候选组”与“子连续组iG”有3次以上的重合，则当前候选帧过关？？？？有问题！！！
                //
        ...
        // 如果该“子候选组i”的所有关键帧都不存在于所有“子连续组”，那么vCurrentConsistentGroups将为空！！！
        // 于是就把“子候选组i”全部拷贝到vCurrentConsistentGroups，并最终用于更新mvConsistentGroups，计数器设为0，重新开始
...
/**
 * @brief 计算当前帧与闭环候选帧的Sim3变换以及闭环候选帧在世界坐标系的sim3位姿等
 *遍历所有闭环候选帧mvpEnoughConsistentCandidates：
	粗估计：求解其与当前帧的sim3,得到当前帧到候选关键帧的位姿。
	精确估计：利用位姿匹配两帧（弥补漏匹配），并更新两帧之间的MapPoint（为后边g2o准备）;
            进行g2o优化位姿,得到候选帧到当前帧的位姿gScm以及匹配点；
            若优化后匹配内点书大于20,则终于得到世界坐标系到当前帧的Sim3变换。
            以及优化后的MapPoint点。跳出整个循环（就认定该候选帧mpMatchedKF，mvpCurrentMatchedPoints）
	检验：取出闭环帧的相连关键帧，得到他们的MapPoint放入mvpLoopMapPoints，表明该MapPoint在闭环时被当前帧观测到。
	寻找更多匹配点检验闭环：将mvpLoopMapPoints中的点全部投影到当前关键帧，查找更多匹配点，匹配成功的点存入mvpCurrentMatchedPoints。如果匹配成功的点>=40,则闭环成功；否则闭环失败，删除当前帧和所有闭环候选帧；
...
bool LoopClosing::ComputeSim3()
{
    ...
    vpSim3Solvers.resize(nInitialCandidates);// 每个候选帧都配有一个Sim3Solver
    ...
    {   //依次给满足匹配要求的闭环候选帧迭代求解sim3(粗估计),
        for(int i=0; i<nInitialCandidates; i++)
        {
            ...
            cv::Mat Scm  = pSolver->iterate(5,bNoMore,vbInliers,nInliers);   //粗估计

            // If Ransac reachs max. iterations discard keyframe
            // 经过n次循环，每次迭代5次，总共迭代 n*5 次

            // 求解失败。总迭代次数达到最大限制还没有求出合格的Sim3变换，该候选帧剔除
            if(bNoMore)
            {
                vbDiscarded[i]=true;
                nCandidates--;
            ...
            //该帧求解成功了，进行精确估计。如果该帧精确估计成功，则跳出循环
            if(!Scm.empty())
            {
                ...
                if(nInliers>=20)
                {
                    ...
                    //mvpCurrentMatchedPoints在闭环时，将闭环候选帧及其相连帧的MapPoint投影到当前帧，与当前帧匹配上的MapPoint集合
    ...
    // 没有一个闭环匹配候选帧通过Sim3的求解与优化，将当前帧和闭环候选帧全部擦掉
    if(!bMatch)
    {
    ...
    // 注意是匹配上的那个闭环帧：mpMatchedKF
    // 将mpMatchedKF相连的关键帧全部取出来放入vpLoopConnectedKFs
    // 将vpLoopConnectedKFs的MapPoints取出来放入mvpLoopMapPoints并作标记。
    ...
    // 步骤8：判断当前帧与闭环关键帧是否有足够多的MapPoints匹配
    //mvpCurrentMatchedPoints在闭环时，将闭环候选帧及其相连帧的MapPoint投影到当前帧，与当前帧匹配上的MapPoint集合
    int nTotalMatches = 0;
    ...
    // 步骤9：清空mvpEnoughConsistentCandidates中除了闭环关键帧之外的所有帧；或者清空所有帧，闭环失败
    if(nTotalMatches>=40)
    {
        for(int i=0; i<nInitialCandidates; i++)
...
void LoopClosing::CorrectLoop()
{
    ...
    // 取出与当前帧相连的关键帧，包括当前关键帧   //这是未闭环前的连接（旧），在后面要删除
    mvpCurrentConnectedKFs = mpCurrentKF->GetVectorCovisibleKeyFrames();
    mvpCurrentConnectedKFs.push_back(mpCurrentKF);
    //经过闭环修正，没有经过闭环修正 的关键帧与位姿
    KeyFrameAndPose CorrectedSim3, NonCorrectedSim3;
    // 先将mpCurrentKF的Sim3变换存入，固定不动（已优化）
    CorrectedSim3[mpCurrentKF]=mg2oScw;    //相对世界坐标系的Sim3变换
    cv::Mat Twc = mpCurrentKF->GetPoseInverse();  //世界坐标系位姿

    {
        ...
        for(vector<KeyFrame*>::iterator vit=mvpCurrentConnectedKFs.begin(), vend=mvpCurrentConnectedKFs.end(); vit!=vend; vit++)
        {
            ...
            if(pKFi!=mpCurrentKF)
            {
                ...
                //其它的关键帧根据相对关系得到Sim3变换(当前帧的位姿固定不动，在computeSim3中已优化得到)
                g2o::Sim3 g2oCorrectedSiw = g2oSic*mg2oScw;
                // Pose corrected with the Sim3 of the loop closure
                // 得到闭环g2o优化后各个相连关键帧的位姿
        ...
        for(KeyFrameAndPose::iterator mit=CorrectedSim3.begin(), mend=CorrectedSim3.end(); mit!=mend; mit++)
        {
            ...
            for(size_t iMP=0, endMPi = vpMPsi.size(); iMP<endMPi; iMP++)
            {
                ...
                // 将该未校正的eigP3Dw先从世界坐标系映射到未校正的pKFi相机坐标系，然后再反映射到校正后的世界坐标系下(什么意思？）
                ...
                pMPi->mnCorrectedByKF = mpCurrentKF->mnId;  //表示该点被当前关键帧闭环修正过
        ...
        for(size_t i=0; i<mvpCurrentMatchedPoints.size(); i++)
        {
            if(mvpCurrentMatchedPoints[i])
            {
                ...
                else// 如果当前帧没有该MapPoint，则直接添加
                {
                    ...
                    pLoopMP->ComputeDistinctiveDescriptors();   //计算具有代表的描述子
                }
            }
        }
    }
    ...
    {
        KeyFrame* pKFi = *vit;
        // 步骤5.2：得到与当前帧相连关键帧pKFi的相连关键帧（二级相连）
        vector<KeyFrame*> vpPreviousNeighbors = pKFi->GetVectorCovisibleKeyFrames();

        // Update connections. Detect new links.
        // 步骤5.3：更新一级相连关键帧的连接关系
        pKFi->UpdateConnections();
        // 步骤5.4：取出该帧更新后的连接关系(当前帧的一级相连帧pKFi的相连帧集合)
        LoopConnections[pKFi]=pKFi->GetConnectedKeyFrames();
        // 步骤5.5：从连接关系中去除闭环之前的二级连接关系，剩下的连接就是由闭环得到的连接关系
        //删除pKFi旧的连接？？？为什么要删除？
        for(vector<KeyFrame*>::iterator vit_prev=vpPreviousNeighbors.begin(), vend_prev=vpPreviousNeighbors.end(); vit_prev!=vend_prev; vit_prev++)
        {
            LoopConnections[pKFi].erase(*vit_prev);  //因闭环而得到的连接关系
        }
...
void LoopClosing::SearchAndFuse(const KeyFrameAndPose &CorrectedPosesMap)
{
    ...
    for(KeyFrameAndPose::const_iterator mit=CorrectedPosesMap.begin(), mend=CorrectedPosesMap.end(); mit!=mend;mit++)
    {
        ...
        // 将闭环相连帧的MapPoints坐标变换到pKFi帧坐标系，然后投影，检查冲突并融合
